...
##################################################################
def spectral_grid_from_wavenumbers(wn_min=0, wn_max=3200, wn_spacing=0.05, wn_num=None):
...
def create_arts_atm(pressure, temperature, gas_concs):
    ...
    atmfield.data[1, :, 0, 0] = 8500 * np.log(
        pressure[0] / pressure
    )  # scale height in [m]
...
def calc_olr_wn(
    ...
    fgrid = spectral_grid_from_wavenumbers(wn_min, wn_max, wn_num=wn_num)
    freq, olr = calc_olr(
        atm, basename=str(arts_data_root.joinpath("lines")) + "/", **fgrid
    )
